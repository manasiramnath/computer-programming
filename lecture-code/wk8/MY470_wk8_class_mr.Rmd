...
ls <- c()
for (i in 1:100){
    ifelse(i %% 3 == 0, ls <- append(ls, NA), ls <- append(ls, i))
...
summ <- 0
for (i in lst){
    ifelse(i %% 2 == 0, summ <- summ + i, summ <- summ)
}
```
...
```{r}
scientists <- c("Alan Turing" = "mathematician", "Richard Feynman" = "physicist",
              "Marie Curie" = "chemist", "Charles Darwin" = "biologist",
              "Ada Lovelace" = "mathematician", "Werner Heisenberg" = "physicist")
print_professions <- function(dic){
    for (i in names(dic)){
        print(paste(i, "was a", dic[i], "."))
    }
}
print_professions(scientists)
```
...
lst <- c(1, 3, 2, 4.5, 7, 8, 10, 3, 5, 4, 7, 3.33)
lst[! lst%%1]^2
newlst <- lst^2 #faster and more legible 
```
